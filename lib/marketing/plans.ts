...
export function buildKanbanCards(sku: string) { return ['Amazon PPC plan','Meta Ads plan','Tracking plan','Creative brief','Launch gate review'].map((title, index) => ({ client_key: `${sku}:${index + 1}`, sku, title, agent: 'marketing-readiness', status: 'preview', mode: 'dry-run', created_at: now() })); }
export function buildIdempotencyKey(sku: string, organizationId: string) { return `${organizationId}:${sku}:kanban-v1`; }
