...
export default function MarketingSkuClient({ sku }: { sku: string }) {
  ...
  async function submitApproval() {
    setSubmitting(true);
    try {
      const r = await fetch('/api/marketing-approvals', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sku, decision, comments }) });
      const d = await r.json();
